    ...
    for dataset in DATASETS:
        ...
        if not data_path.exists():
            raise FileNotFoundError(
                f"Missing Correia CSV: {data_path}. Download the collection from "
                "https://scorreia.com/data/hdfe/index.html into data/correia_data/ "
                "(see data/correia_data/README.md)."
            )
        ...
        rows.append(
            {
                ...
                "n_runs": n_runs,
                "n_success": n_success,
                "time": successful["time"].median() if n_success else pd.NA,
                "success": success,
    ...
    return pd.DataFrame(
        {
            ...
            "n_runs": results_df["n_runs"],
            "n_success": results_df["n_success"],
            "model_k": len(SPEC.covariates),
            "model": SPEC.formula,
