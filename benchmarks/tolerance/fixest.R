...
fit_once <- function(tolerance) {
  fit <- feols(
    ...
    fixef.iter = maxiter, nthreads = threads, warn = FALSE, notes = FALSE
  )
...
warmup_fit <- fit_once(tolerances[[1]])
rm(warmup_fit)
rows <- list()
index <- 1L
  ...
  for (repetition in seq_len(repetitions)) {
    ...
    rows[[index]] <- tryCatch({
      ...
      data.frame(
        ...
        converged = FALSE, capped = grepl("iter|converg|maxiter", message, ignore.case = TRUE),
        error = message
      )
    })
    index <- index + 1L
